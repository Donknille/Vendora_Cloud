...
import {
  ...
  createMarketFormStateFromMarket,
...
export default function EditMarketScreen() {
  ...
  const [hydratedMarketId, setHydratedMarketId] = useState<string | null>(null);

  useEffect(() => {
    if (!market || hydratedMarketId === market.id) {
      return;
    }

    setFormState(createMarketFormStateFromMarket(market));
    setHydratedMarketId(market.id);
  }, [hydratedMarketId, market]);
  ...
  const saveMarket = async () => {
    ...
    try {
      await updateMarket.mutateAsync({
        id,
        data: buildMarketMutationInput(formState),
      });
      ...
      >
        ...
        <Pressable
          ...
          style={({ pressed }) => [
            ...
            (pressed || updateMarket.isPending) && { opacity: 0.8 },
          ]}
        >
