...
export default function MarketsScreen() {
  ...
  const { data: markets = [], refetch: refetchMarkets, isRefetching: isRefetchingMarkets } =
    useMarketsQuery();
  const { data: allSales = [], refetch: refetchSales, isRefetching: isRefetchingSales } =
    useMarketSalesQuery();

  const isRefetching = isRefetchingMarkets || isRefetchingSales;

  const salesMap = allSales.reduce<Record<string, typeof allSales>>((acc, sale) => {
    if (!acc[sale.marketId]) {
      acc[sale.marketId] = [];
    }
    acc[sale.marketId].push(sale);
    return acc;
  }, {});
  ...
  const renderMarket = ({ item, index }: { item: (typeof markets)[number]; index: number }) => {
    const sales = salesMap[item.id] || [];
    const totalSales = sales.reduce((sum, sale) => sum + sale.amount * sale.quantity, 0);
    const totalCosts = (item.standFee || 0) + (item.travelCost || 0);
